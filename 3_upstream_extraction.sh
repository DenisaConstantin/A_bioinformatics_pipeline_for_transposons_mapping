...
sed -i -e "s/>/\>Coordinate: $coordinate1. Strand: $strand. Range: $START:$STOP Length: /g" $file_base_name$coordinate1.$strand$extension
fi
...
then
  ...
  then
  START=1 
  if ! [ $coordinate1 -le $LengthFL2 ]
  then
  START=$( expr $coordinate1 - $LengthFL2 + 1 )
  fi
  
  if [ $( expr $coordinate1 - $LengthFL2 + 1 ) -lt $coordinate2 ]
  then
  START=$coordinate2
  fi
  
  STOP=$( expr $coordinate1 + $LengthFL1 )
...
sed -i -e "s/>/\>Coordinate: $coordinate1. Strand: $strand. Range: $START:$STOP Length: /g" $file_base_name$coordinate1.$strand$extension
  ...
  sed -i -e "s/>/\>Coordinate: $coordinate2. Strand: $strand. Range: $START:$STOP Length: /g" $file_base_name$coordinate2.$strand$extension
  fi
